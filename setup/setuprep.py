...
def run(rep, nreps, maindir, repdir, rnseed):
    ...
    inner_rbdir        = os.path.join(repdir, 'rb')
    ...
    outer_rbdir        = os.path.join(maindir, inner_rbdir)
   ...
   ##############################
    ...
    # that have all-missing taxa removed           #
    ################################################
    segregateLocusData(os.path.join(outer_simdir, 'sim.nex'), outer_rbdir, subset_info)
