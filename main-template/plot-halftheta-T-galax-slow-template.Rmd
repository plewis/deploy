...
info <- c(__SMC_INFO__)
t_ratio <- T / halftheta
infodf <- data.frame(halftheta, T, info, smcmean, t_ratio, slowloci)
...
3D plot labeled by nslowloci

```{r}
library(plotly)
fig <- plot_ly(infodf, x = ~info, y = ~smcmean, z = ~slowloci, size=1, mode = "markers", color=slowloci)
fig 
