...
# Create vec slow loci
slow_loci = []
for s in summary:
    slow_loci.append('%g' % s['nslowloci'])
...
if plot_theta_vs_lambda:
    ...
    stuff = open('plot-halftheta-T-galax-slow-template.Rmd', 'r').read()
    stuff = re.sub('__NSPECIES__', '%d' % nspecies, stuff, re.M | re.S)
